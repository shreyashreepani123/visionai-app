import os
import torch
import torch.nn as nn
import torch.nn.functional as F
import torchvision.transforms as T
import torchvision.models.segmentation as segmodels
import numpy as np
import cv2
from PIL import Image
import streamlit as st
import requests

# ---------------- CONFIG ----------------
CHECKPOINT_PATH = "checkpoint.pth"
DEVICE = torch.device("cpu")
IMAGE_SIZE = 256  # Inference size, will resize then back

# ---------------- MODEL ----------------
def build_deeplab(num_classes=91, aux_loss=False):
    model = segmodels.deeplabv3_resnet101(weights=None, aux_loss=aux_loss)
    model.classifier[4] = nn.Conv2d(256, num_classes, kernel_size=1)
    if getattr(model, "aux_classifier", None) is not None:
        model.aux_classifier[4] = nn.Conv2d(256, num_classes, kernel_size=1)
    return model

@st.cache_resource
def load_model():
    model = build_deeplab(num_classes=91)
    state = torch.load(CHECKPOINT_PATH, map_location=DEVICE)
    model.load_state_dict(state, strict=False)
    model.eval().to(DEVICE)
    return model

# ---------------- TRANSFORM ----------------
transform = T.Compose([
    T.Resize((IMAGE_SIZE, IMAGE_SIZE)),
    T.ToTensor(),
    T.Normalize(mean=[0.485, 0.456, 0.406],
                std=[0.229, 0.224, 0.225]),
])

# ---------------- POSTPROCESS ----------------
def postprocess_logits_multiclass(logits, orig_h, orig_w, image_np):
    probs = torch.softmax(logits, dim=1)  # (1, 91, h, w)
    up = F.interpolate(probs, size=(orig_h, orig_w), mode="bilinear", align_corners=False)
    mask = up.argmax(1).squeeze(0).cpu().numpy().astype(np.uint8)  # (H, W)

    # Binary mask (objects vs background)
    binary = (mask > 0).astype(np.uint8) * 255

    # Original colors mask (objects keep original pixel colors)
    color_mask = np.zeros_like(image_np)
    color_mask[mask > 0] = image_np[mask > 0]

    # Multi-class colored mask
    colors = np.random.randint(0, 255, size=(91, 3), dtype=np.uint8)
    class_colored = colors[mask]

    return binary, color_mask, class_colored

# ---------------- STREAMLIT APP ----------------
st.title("VisionAI Segmentation App (COCO 91 Classes)")

uploaded_file = st.file_uploader("Upload an image", type=["jpg", "jpeg", "png"])
if uploaded_file is not None:
    image = Image.open(uploaded_file).convert("RGB")
    image_np = np.array(image)
    orig_h, orig_w = image_np.shape[:2]

    st.image(image, caption="Uploaded Image", use_column_width=True)

    model = load_model()

    # Preprocess
    input_tensor = transform(image).unsqueeze(0).to(DEVICE)

    # Inference
    with torch.no_grad():
        logits = model(input_tensor)["out"]

    # Postprocess
    binary, color_mask, class_colored = postprocess_logits_multiclass(logits, orig_h, orig_w, image_np)

    st.subheader("Binary Mask (objects vs background)")
    st.image(binary, clamp=True, channels="GRAY")

    st.subheader("Color Mask (original colors on black background)")
    st.image(color_mask, clamp=True)

    st.subheader("Colored Multi-class Mask (visualized)")
    st.image(class_colored, clamp=True)







